...
BRAND_MODELS = {
    "Chevrolet": ["Agile", "Astra", "Celta", "Classic", "Cobalt", "Corsa", "Cruze", "Joy", "Montana", "Onix", "Prisma", "S10", "Spin", "Tracker", "Vectra", "Zafira"],
    "Citroen": ["Aircross", "C3", "C4", "C4 Cactus", "Xsara Picasso"],
    "Fiat": ["Argo", "Bravo", "Cronos", "Doblo", "Fiorino", "Grand Siena", "Idea", "Mobi", "Palio", "Pulse", "Punto", "Siena", "Strada", "Toro", "Uno"],
    "Ford": ["Courier", "EcoSport", "Edge", "Fiesta", "Focus", "Fusion", "Ka", "Ranger"],
    "Honda": ["City", "Civic", "CR-V", "Fit", "HR-V", "WR-V"],
    "Hyundai": ["Creta", "HB20", "HB20S", "i30", "ix35", "Santa Fe", "Tucson"],
    "Jeep": ["Commander", "Compass", "Renegade"],
    "Kia": ["Cerato", "Picanto", "Sorento", "Soul", "Sportage"],
    "Mitsubishi": ["ASX", "Eclipse Cross", "L200", "Outlander", "Pajero", "TR4"],
    "Nissan": ["Frontier", "Kicks", "Livina", "March", "Sentra", "Tiida", "Versa"],
    "Peugeot": ["2008", "206", "207", "208", "3008", "307", "308"],
    "Renault": ["Captur", "Clio", "Duster", "Kangoo", "Kwid", "Logan", "Megane", "Oroch", "Sandero", "Scenic", "Symbol"],
    "Toyota": ["Bandeirante", "Corolla", "Corolla Cross", "Etios", "Hilux", "RAV4", "SW4", "Yaris"],
    "Volkswagen": ["Amarok", "Fox", "Gol", "Golf", "Jetta", "Nivus", "Parati", "Polo", "Saveiro", "SpaceFox", "T-Cross", "Taos", "Tiguan", "Up", "Virtus", "Voyage"],
}
BRANDS = sorted(BRAND_MODELS)
...
def delete_row(table: str, row_id: str) -> None:
    if is_demo():
        init_demo_data()
        st.session_state[table] = [item for item in st.session_state[table] if item["id"] != row_id]
        if table == "vehicles":
            for child_table in ["vehicle_photos", "expenses", "proposals", "sales"]:
                st.session_state[child_table] = [
                    item for item in st.session_state[child_table] if item.get("vehicle_id") != row_id
                ]
        return
    supabase_request("DELETE", table, params={"id": f"eq.{row_id}"}, prefer="return=minimal")
...
def selected_or_custom(label: str, options: list[str], key: str) -> str:
    choices = [""] + options + ["Outra opcao"]
    selected = st.selectbox(label, choices, key=f"{key}-select")
    if selected == "Outra opcao":
        return st.text_input(f"Digite {label.lower()}", key=f"{key}-custom").strip()
    return selected
...
def sold_template_image() -> Image.Image:
    if SOLD_TEMPLATE_PATH.exists():
        return Image.open(SOLD_TEMPLATE_PATH).convert("RGB")
    template = Image.new("RGB", (1080, 1080), "#09090b")
    draw = ImageDraw.Draw(template)
    draw.rectangle((0, 0, 1080, 150), fill="#8f0d12")
    draw.rectangle((0, 150, 1080, 340), fill="#5d070a")
    draw.rectangle((38, 267, 1042, 909), outline="#ffffff", width=10)
    draw.rectangle((0, 920, 1080, 1080), fill="#0d0d0f")
    draw.text((86, 88), "VENDIDO", fill="#ffffff", font=ImageFont.load_default(size=64))
    draw.text((86, 175), "+1 CLIENTE SATISFEITO", fill="#ffffff", font=ImageFont.load_default(size=54))
    draw.text((705, 945), "ENTRE EM CONTATO", fill="#ffffff", font=ImageFont.load_default(size=24))
    draw.text((640, 978), "(62) 98246-3698", fill="#ffffff", font=ImageFont.load_default(size=42))
    if LOGO_PATH.exists():
        logo = Image.open(LOGO_PATH).convert("RGBA")
        logo.thumbnail((300, 120))
        template.paste(logo, (90, 940), logo)
    return template


def build_sold_post(car_photo: Image.Image) -> bytes:
    template = sold_template_image()
    fitted_photo = cover_resize(car_photo, (984, 622))
    template.paste(fitted_photo, (48, 277))
...
def vehicles_page() -> None:
    ...
    if rows:
        ...
        available_brands = sorted({vehicle.get("brand", "") for vehicle in rows if vehicle.get("brand")})
        filter_cols = st.columns(2)
        status_filter = filter_cols[0].multiselect("Filtrar por status", VEHICLE_STATUS, default=VEHICLE_STATUS)
        brand_filter = filter_cols[1].selectbox("Filtrar por marca", ["Todas"] + available_brands)
        filtered_rows = [vehicle for vehicle in rows if vehicle.get("status") in status_filter]
        if brand_filter != "Todas":
            filtered_rows = [vehicle for vehicle in filtered_rows if vehicle.get("brand") == brand_filter]
...
def vehicle_profile_page(vehicle_id: str) -> None:
    ...
    is_sold = str(vehicle.get("status", "")).strip().lower() == "vendido" or new_status == "Vendido"
    if is_sold:
    ...
    with left:
        ...
        with st.expander("Excluir veiculo"):
            st.warning("Excluir o veiculo tambem remove fotos, custos, propostas e vendas vinculadas.")
            confirm_delete = st.checkbox(
                f"Confirmo que quero excluir {vehicle_display_name(vehicle)}",
                key=f"confirm-delete-{vehicle_id}",
            )
            if st.button("Excluir veiculo", disabled=not confirm_delete):
                delete_row("vehicles", vehicle_id)
                st.session_state.selected_vehicle_id = None
                if "vehicle_id" in st.query_params:
                    del st.query_params["vehicle_id"]
                st.success("Veiculo excluido.")
                st.rerun()
    with right:
